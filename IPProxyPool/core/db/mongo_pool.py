...
目标：实现对数据库增删改查相关操作
步骤：
1. 在init中，建立数据连接，获取要操作的结合，在del方法中关闭数据库连接
...
class MongoPool(object):
    ...
    def insert_one(self,proxy):
        """
        实现插入功能，使用proxy.ip作为MonegoDB中数据的主键: _id
        :param proxy:
        :return:
        """
        #print(type(proxy))  # <class 'domain.Proxy'>
        ...
        if count == 0:
            ...
            logger.warning(f"已经存在的代理:{proxy}")
    ...
    def find(self,conditions= {},count=0):
        ...
        """
        cursor = self.proxies.find(conditions).sort([("score",pymongo.DESCENDING),("speed",pymongo.ASCENDING)]).limit(count)
    ...
    def random_proxy(self,protocol=None,domain=None,count=0,nick_type=2):
        """
        3.3 实现根据协议类型 和 要访问网站的域名，获取代理IP列表
        :param protocol: 协议：http，https
        :param domain:  域名：jd.com
        :param count: 用于限制获取多个代理IP，默认是获取所有的
        :param nick_type: 匿名类型，默认，获取高匿的代理IP
...
if __name__ == '__main__':
    mongo = MongoPool()
    # proxy = Proxy('14.207.175.80', port='8888')
    # mongo.insert_one(proxy)
    # for proxy in mongo.get_proxies(protocol='http',domain='baidu.com'):
    #     print(proxy)
    # print(mongo.disable_domain('14.207.175.80','baidu.com'))
    proxy = mongo.get_proxies('http','jd.com',5)